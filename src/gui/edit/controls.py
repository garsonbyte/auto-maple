...
class Controls(Frame):
    def __init__(self, parent, **kwargs):
        ...
        self.up_arrow = Button(self, text='▲', width=6, command=self.move('up'))
        self.up_arrow.grid(row=0, column=0)

        self.down_arrow = Button(self, text='▼', width=6, command=self.move('down'))
        self.down_arrow.grid(row=0, column=1, padx=(5, 0))

        self.delete = Button(self, text='\U00002715', width=3, command=self.delete)
        self.delete.config(foreground='red')
        self.delete.grid(row=0, column=2, padx=(5, 0))

        self.new = Button(self, text='\U00002795', width=6, command=self.new)
        self.new.config(foreground='lime green')
